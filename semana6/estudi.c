...
{//declaracion de variables y arreglos
	...
	ar = fopen("est.txt","w");
	fputs("EL resumen es: \t",ar);
	for(l=0;l<=8;l++){
		fprintf(ar,"El numero total de alumnos en el semestre: ");
		m=l+1;
		fprintf(ar,"%d \n",m);
		fprintf(ar,"%d \n",arr[l]);
	}
	k=prom/10;//variable donde se guarda el promedio
		fprintf(ar,"Se capturaron 10 estudiantes, de los cuales %d son mujeres y %d son hombres \n",mujer,hombre);
		fprintf(ar,"El promedio de calificaciones de todos los estudiantes es: %f",k);
		fclose(ar);
}
